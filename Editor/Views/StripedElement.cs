...
{
	/// <summary>
	/// A VisualElement with a striped background.
	/// </summary>
	internal sealed class StripedElement : VisualElement
	{
		private static readonly CustomStyleProperty<Color> s_stripeColor = new("--stripe-color");
		private static readonly CustomStyleProperty<float> s_stripeWidth = new("--stripe-width");
		private static readonly CustomStyleProperty<float> s_stripeSpacing = new("--stripe-spacing");
		private static readonly CustomStyleProperty<float> s_stripeAngle = new("--stripe-angle");

		/// <summary>
		/// The color used by the stripe. Alternates with transparent.
		/// </summary>
		[PublicAPI]
		public Color StripeColor { get; set; } = Color.black;

		/// <summary>
		/// The width of the stripe, alternates with <see cref="StripeSpacing"/>.
		/// </summary>
		[PublicAPI]
		public float StripeWidth { get; set; } = 10;

		/// <summary>
		/// The width of the space between stripes, alternates with <see cref="StripeWidth"/>.
		/// </summary>
		[PublicAPI]
		public float StripeSpacing { get; set; } = 10;

		/// <summary>
		/// The angle the stripes face in degrees. 0 is straight up, and the direction moves clockwise.
		/// </summary>
		[PublicAPI]
		...
		public StripedElement()
		{
			...
			{
				var modified = false;
				if (evt.customStyle.TryGetValue(s_stripeColor, out Color color))
				{
					StripeColor = color;
					modified = true;
				}

				if (evt.customStyle.TryGetValue(s_stripeWidth, out float width))
				{
					StripeWidth = width;
					modified = true;
				}

				if (evt.customStyle.TryGetValue(s_stripeSpacing, out float spacing))
				{
					StripeSpacing = spacing;
					modified = true;
				}

				if (evt.customStyle.TryGetValue(s_stripeAngle, out float angle))
				{
					StripeAngle = angle;
					modified = true;
				}
